...
// Loaded ONLY after consent (the parent gates this). Each provider is included
// only when its env id is present, so Meta/GA4 stay dormant until you add ids.
"use client";

import Script from "next/script";
import { PIXELS } from "@/lib/analytics/track";
      ...
      {PIXELS.ga4 && (
        <>
          ...
          <Script id="ga4-init" strategy="afterInteractive">{`
            ...
            gtag('js', new Date());
            gtag('config', '${PIXELS.ga4}', { send_page_view: false });
          `}</Script>
        </>
      )}
    </>
